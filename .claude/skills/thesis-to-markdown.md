---
name: thesis-to-markdown
description: 把任意输入（粘贴文本、草稿、.docx/.pdf/.txt 文件、或已有 input/*.md）转成 SSPU 毕业论文解析器严格接受的 Markdown。当用户说"写论文""整理论文""转 markdown""把这个变成论文""从 docx 转""帮我起个稿""thesis to markdown""起草毕业论文""把这段放进论文"时触发。即使用户只是丢了一段文字没说格式，只要上下文是毕业论文，也应触发。
---
...
- **A**: 用 AskUserQuestion 收集：题目、专业、大致章节骨架（至少 4 章）、研究背景一句话概述。然后基于这些信息**起草全文结构**（不要等用户写）。
- **B**: 直接读粘贴文本。
- **C**: 用 Bash 提取：
...
能从原文抓就抓，抓不到就用 **AskUserQuestion 一次性补齐所有缺失项**（不要分多次问）。
...
1. 用 AskUserQuestion 问："是否需要用 humanizer-academic-zh 去除 AI 痕迹？"
   - Yes → `Skill(skill="humanizer-academic-zh", args="input/<slug>.md")`
   - No → 跳过
2. 最后输出固定文案：
...
3. 用户同意后，用 `ctx_execute` 或 Bash 运行 matplotlib 脚本，**严格使用上述黑白风格**：
   ```python
...
5. **双平台同步**：本文件内容与项目根目录 `AGENTS.md` 逐字同步（Codex 用后者）。改一处必改另一处。
6. **范文参考**：`input/thesis.md` 是合规范例，不确定时 Read 它对照。
